...
describe User do 
	...
	describe "#favorited(post)" do
		...
		it "returns the appropriate favorite if it exists" do
			...
			topic = Topic.create! name: 'example', description: 'This is a topic for this spec test'
			post = Post.create! title: 'This is a title for this', body:'This is a body for the title of this post for this test' # create a post
			favorite = Favorite.create! user:'user_example'# create a favorite for the user for that post

			favorited = user.favorite(post)# call favorited on the user with that post in parameters

			expect(favorited).to eq(favorited) # expect the result to be the favorite
		end
	end
end